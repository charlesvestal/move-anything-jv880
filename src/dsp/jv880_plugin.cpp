...
/* Sample rates */
/* JV-880 PCM runs at 32 kHz base rate (64 kHz only with oversampling enabled). */
#define JV880_SAMPLE_RATE 32000
...
static void *emu_thread_func(void *arg) {
    ...
    /* Linear interpolation resampler for 32k → 44.1k upsampling.
     * resample_pos tracks position in input samples (fractional).
     * Each output sample is interpolated between prev and curr input samples. */
    const double step = (double)JV880_SAMPLE_RATE / (double)MOVE_SAMPLE_RATE;
    double resample_pos = 0.0;
    int32_t prev_l = 0, prev_r = 0;
    int32_t curr_l = 0, curr_r = 0;
    int input_idx = 0;
    int avail = 0;
    ...
    while (g_thread_running) {
        ...
        g_mcu->updateSC55(64);
        avail = g_mcu->sample_write_ptr;
        input_idx = 0;

        /* Generate output samples with linear interpolation */
        while (ring_free() > 0) {
            /* Advance input samples as needed */
            while (resample_pos >= 1.0 && input_idx < avail) {
                resample_pos -= 1.0;
                prev_l = curr_l;
                prev_r = curr_r;
                curr_l = g_mcu->sample_buffer[input_idx];
                curr_r = g_mcu->sample_buffer[input_idx + 1];
                input_idx += 2;
            }
...
static void *load_thread_func(void *arg) {
    ...
    /* Pre-fill audio buffer with linear interpolation */
    g_ring_write = 0;
    g_ring_read = 0;
    ...
    for (int i = 0; i < 256; i++) {
        ...
        while (g_ring_write < AUDIO_RING_SIZE / 2) {
            /* Advance input samples as needed */
            while (resample_pos >= 1.0 && input_idx < avail) {
                resample_pos -= 1.0;
                prev_l = curr_l;
